...
namespace AshfallFrontier.UI
{
    ...
    public class HudAutoBuilder : MonoBehaviour
    {
        ...
        [Header("Layout")]
        public Vector2 marginTopLeft = new Vector2(20, 20);
        ...
        void Start()
        {
            ...
            prt.anchoredPosition = new Vector2(marginTopLeft.x - 10f, -marginTopLeft.y + 10f);
            prt.sizeDelta = new Vector2(size.x + 20, (size.y + gap) * 3 + 20);

            var hp = MakeBar(panelGo.transform, "HP", 0, new Color(0.85f, 0.2f, 0.2f));
            var st = MakeBar(panelGo.transform, "Stamina", 1, new Color(0.2f, 0.85f, 0.2f));
            var ma = MakeBar(panelGo.transform, "Mana", 2, new Color(0.25f, 0.45f, 0.95f));
        ...
        Slider MakeBar(Transform parent, string label, int index, Color fillColor)
        {
            // Root
            ...
            float x = marginTopLeft.x;
            float y = marginTopLeft.y + index * (size.y + gap);
